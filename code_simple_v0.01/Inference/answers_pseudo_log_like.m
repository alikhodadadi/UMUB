function [ log_like ] = answers_pseudo_log_like( data, model )
...
tmodel.a_epsilons     = model.var_pars.a_epsilons; %answers variational pars 
...
for i = 1 :  length(events.answrs)
    ...
    epsi = tmodel.a_epsilons(i); 
    event_counts(ui, 2) = event_counts(ui, 2) + 1;
    [ I ] = answer_intensity ...
        ( model, event_counts, ti,events.quests.times, events.quests.tags,@g);
    q_lgs = q_lgs + log(I(ui)) + ...
        calc_parent_pseudo_log_prob(ti, ui, pi, user_expertises(ui,:),epsi, events);
end
lambdas_int = 0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function lprob = calc_parent_pseudo_log_prob(t, u, i, u_expertise,ei, events)
